...
function MailIcon() { 
  return ( 
    <svg viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2.2' strokeLinecap='round' strokeLinejoin='round' className='h-6 w-6' > 
      <rect x='3' y='5' width='18' height='14' rx='2' /> <path d='m4 7 8 6 8-6' /> </svg> 
  ) 
} 
function GlobeIcon() { 
  return ( 
    <svg viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2.2' strokeLinecap='round' strokeLinejoin='round' className='h-6 w-6' > <circle cx='12' cy='12' r='9' /> 
  ...
  function Contact({ contact }: ContactProps) { 
    return ( 
      <section id='contact' className='scroll-mt-28 mx-auto max-w-6xl px-6 py-24' > 
        <div className='mb-12 max-w-3xl'> <p className='text-sm uppercase tracking-[0.25em] text-zinc-500'> {contact.eyebrow} </p> 
          <h2 className='mt-3 text-3xl font-semibold leading-tight text-white md:text-5xl'> {contact.title} </h2> 
          <p className='mt-6 text-base leading-8 text-zinc-300 md:text-lg'> {contact.description} </p> </div> 
        <div className='grid gap-6 md:grid-cols-2'> {contact.items.map(item => { const isMail = item.key === 'gmail' 
        return ( 
          <a key={item.key} href={item.href} target={isMail ? undefined : '_blank'} rel={isMail ? undefined : 'noreferrer'} className='group flex items-center gap-4 rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm transition-all duration-300 hover:border-white/20 hover:bg-white/[0.07]' > 
            <div className='relative flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-2xl text-white'> 
              ...
              <span className='relative z-10 text-white'> {iconMap[item.key]} </span> </div> 
            <div>
